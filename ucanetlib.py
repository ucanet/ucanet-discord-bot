...
def start_git():
    ...
    if is_git_repo(GIT_PATH):
        ...
        print("[DEBUG] Git repo already exists.")
    else:
    ...
    if 'origin' in [remote.name for remote in repo.remotes]:
        ...
        print(f"[DEBUG] Found existing remote: {origin.url}")
    else:
        origin = repo.create_remote('origin', GIT_URL)
        print(f"[DEBUG] Created new remote with URL: {GIT_URL}")

    print("[DEBUG] Git remotes:")
...
def pull_git():
    file_lock.acquire()
    try:
        print("[DEBUG] Attempting to pull from remote...")
...
def push_git():
    ...
    if len(pending_changes) > 0:
        print("[DEBUG] Pending changes found, preparing to commit...")
        try:
            formatted_changes = {}
            ...
            print("[DEBUG] Formatted changes:")
            for domain, line in formatted_changes.items():
            ...
            for i, line in enumerate(registry_lines):
                ...
                if split_lines[0] in formatted_changes:
                    registry_lines[i] = formatted_changes.pop(split_lines[0])

            for line in formatted_changes.values():
                if registry_lines and not registry_lines[-1].endswith("\n"):
                    registry_lines[-1] += "\n"
                registry_lines.append(line)

            if registry_lines and registry_lines[-1].endswith("\n"):
                registry_lines[-1] = registry_lines[-1].rstrip("\n")

            with open(REGISTRY_PATH, 'w') as registry_file:
                registry_file.writelines(registry_lines)

            print("[DEBUG] Registry file updated.")

            pending_changes.clear()
        except Exception as e:
            print(f"[ERROR] Failed to apply pending changes: {e}")

        try:
            repo = git.Repo(GIT_PATH)
            repo.config_writer().set_value("user", "email", "bot@example.com").release()
            repo.config_writer().set_value("user", "name", "Bot User").release()

            print("[DEBUG] Adding and committing changes...")
            repo.git.add(all=True)
        ...
        print("[DEBUG] No pending changes to push.")
